        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                for (int i = 0; i < d.places.size(); i++)
                    if (dist(d.places.get(i).lat, d.places.get(i).lon, lat, lon) < MSRUtils.CLUSTER_RADIUS) {
//                        System.out.println();
                        Place p = new Place( d.places.get(i).id, d.places.get(i).label, lat, lon);
                ...
                if (isNew) {
                    d.places.add(new Place(currentLocationIndex, currentLocationIndex + "", lat, lon));
...
//                System.out.println(i + ":: " + cnt[i]);
        ...
        for (int i = 0; i < d.places.size(); i++)
            if (!indexList.contains(d.places.get(i).id)) {
                ...
                d.places.get(i).id = idToIndex.get(d.places.get(i).id);
                d.places.get(i).label = idToIndex.get(d.places.get(i).id) + "";
    ...
    private static boolean inAllPlace(Place place) {
        for(Place p : dAll.places)
            if(p.id!=0 && dist(p.lat, p.lon, place.lat, place.lon) < MSRUtils.CLUSTER_RADIUS)
                return true;
         return false;
