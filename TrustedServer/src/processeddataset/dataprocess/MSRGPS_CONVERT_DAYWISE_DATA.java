...
public class MSRGPS_CONVERT_DAYWISE_DATA {
    ...
    private static void processFile(File input) {
      try {
            ...
            new File(MSRConfigResource.MSR_GPS_PRIVACY_DATA_2009_DAYWISE_DATA_DIR + userId).mkdir();
...
//                  System.out.println(tokens[1] + "," +curDay);
                if (!tokens[4].equals(curDay)) {
                    ...
                    String dy = tokens[4];
                    dy = dy.replaceAll("/", "_");
                    f = new FileWriter(MSRConfigResource.MSR_GPS_PRIVACY_DATA_2009_DAYWISE_DATA_DIR + userId + "\\" + dy + ".csv");
                    firtFile=false;
                    curDay = tokens[4];
                    f.write(s + "\n");
                    numberOfDays++;
             ...
             f = new FileWriter(MSRConfigResource.MSR_GPS_PRIVACY_DATA_2009_DAYWISE_DATA_DIR+"statstics_participant_days.txt");
