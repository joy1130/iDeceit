...
public class obfuscationMthod {
    ...
    double M_all[][][][][];// = new double[labeldata.Utils.T][labeldata.Utils.N][labeldata.Utils.T][labeldata.Utils.N];
...
//              List<LabelPlaceTraces> inputTraces = obf.getLabelPlaceTraces(new File("D:\\Research\\data\\MSR GPS Privacy Dataset 2009\\daywiseDataMemphisLike\\01\\10_26_2009.csv"));
...
//
    ...
    void runEvaluation() {
        ...
        int[] numberReleasedNode = new int[111];
        int[] totalNodes = new int[111];

        List<Integer> userIds = new ArrayList<Integer>();
        try {
            for (int userId = Utils.minUserID; userId < Utils.maxUserID; userId++) {
                userIds.add(userId);

                userUtility[userId] = 0;
                userPlau[userId] = 0;

                numberReleasedNode[userId] = 0;
                totalNodes[userId] = 0;

                userUtilityPerfect[userId] = 0;
                userPlauPerfect[userId] = 0;

                String userIdStr = userId + "";
                if (userId < 10) userIdStr = "0" + userId;

                M = getResedualMC(M_all, userId);

                int userCnt = 0;
                String DIR = "D:\\Research\\data\\MSR GPS Privacy Dataset 2009\\daywiseDataMemphisLike\\" + userIdStr + "\\";
                File folder = new File(DIR);
                for (File inputFile : folder.listFiles()) {
                    String dayId = inputFile.getName();
                    System.out.println("userid:" + userId + " day:" + inputFile.getPath());
...
//                    System.out.println(inputTraces.size());
                    ...
                    totalUtil += utility;
                    if (utility == 1)
                        numberReleasedNode[userId] += (input.size() / 2);
                    totalNodes[userId] += (input.size() / 2);
...
//                    System.out.println(userId + "," + dayId + ":" + utility);
            ...
            try {
                FileWriter f = new FileWriter(new File("D:\\Research\\data\\MSR GPS Privacy Dataset 2009\\mobicom_result_userMC04.csv"));
                f.write("UID,Utility,Plausibility,Utility(Perfect prediction),Plausibility(Perfect prediction),SensitiveInterval TimeInMinute,NonsensitiveInterval TimeInMinute,MaskIt_#release,#node" + "\n");

                for (int uID : userIds) {
                    String s = uID + "," + userUtility[uID] + "," + userPlau[uID] + "," + getPerfect(userUtility[uID]) + "," + getPerfect(userPlau[uID])
                            + "," + sensitiveInterval[uID] + "," + nonSensitiveInterval[uID] + "," +numberReleasedNode[uID]+","+totalNodes[uID] +"\n";
                    f.write(s);
...
//            System.out.println("P Total Utility :" + totalUtilPerfect + "; Count :" + cnt + "; avg Utility :" + totalUtilPerfect / cnt + "; avg Plausibility :" + totalPlauPerfect / cnt);
    ...
    private double[][][][] getResedualMC(double[][][][][] m_all, int userId) {
        double M[][][][] = new double[Utils.T][Utils.N][Utils.T][Utils.N];
        double alpha = 1;
            ...
            for (int i = 0; i < Utils.N; i++)
                for (int n_j = 0; n_j < Utils.T; n_j++)
                    for (int j = 0; j < Utils.N; j++)
                        M[n_i][i][n_j][j] = alpha * M_all[userId][n_i][i][n_j][j] + (1 - alpha) * M_all[0][n_i][i][n_j][j];
        return M;  //To change body of created methods use File | Settings | File Templates.
    }
...
//System.out.println(input.size());
        for (int i = 1; i < input.size() - 2; i += 2)
            ret += M[input.get(i).state.timeIndex][input.get(i).state.place.id][input.get(i + 1).state.timeIndex][input.get(i + 1).state.place.id];
...
//            System.out.println(sameReleasedInterval+"="+totalInterval);
    ...
    public void printEvents(List<Event> input) {
        for (Event e : input) {
            System.out.print("(" + e.state.place.id + "," + e.timestamp + ":" + e.state.timeIndex + "," + e.type + ")->");
        }
    }
...
//        sensitiveNodes = new SensitiveNodes();
        ...
        DT_MAX_all = mc.DT_MAX_all;
        DT_MIN_all = mc.DT_MIN_all;
        R_all = mc.R_all;
        M_all = mc.M_all;
        ...
        for (int n_i = 0; n_i < Utils.T; n_i++)
            for (int i = 0; i < Utils.N; i++) {
                for (int n_j = 0; n_j < Utils.T; n_j++)
                    for (int j = 0; j < Utils.N; j++)
                        M_all[0][n_i][i][n_j][j] = M[n_i][i][n_j][j];
            }
